...
module.exports = async (dni) => dniHandler.operation(dniHandler.token).then(async token => dniHandler.operation(dniHandler.dniSave(dni), token)).then(async data => dniHandler.result(data)).catch(async error => {
  if (error.code === 2) {
    return dniHandler.operation(dniHandler.dniSave(dni, "dniSimple"), error.token).then(data => dniHandler.result(data)).catch(err => {
      throw new Error(err);
    });
  }
