...
class CalificationService extends BaseService {
  ...
  async create(entity) {
    ...
    let calification;
    if (idProduct) calification = await _calificationRepository.getByProduct(idClient, idProduct);else if (idService) calification = await _calificationRepository.getByService(idClient, idService);else if (idBusiness) calification = await _calificationRepository.getByBusiness(idClient, idBusiness);
  ...
  async calificationGet(califications) {
    return Promise.all(califications.map(async element => {
